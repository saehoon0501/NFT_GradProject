...
module.exports = {
 ...
 sndJwt : (req, res, next) => {
    ...
    if(!publicAddress || !signature || !msg)
        return res.status(400).send({error: 'Request should have 3 elements(publicAddr, sig, msg'});
        ...
        .then((usr)=>{
            ...
            return res.json({accessToken: jwt.sign({publicAddress}, config.secretKey, config.options)});
        })
    }
}